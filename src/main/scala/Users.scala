...
class Users {
    ...
    def createUser(username: String, userPassword: String, userMail: String,  role: Option[Role]) = {
        val userId =  UUID.randomUUID.toString

        def persistNewUser(roleId: Option[Int]) = {
            val newUser = User(userId, username, userPassword, userMail, LocalDateTime.now(), roleId)
