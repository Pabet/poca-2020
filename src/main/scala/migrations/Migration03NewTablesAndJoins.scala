...
class Migration03NewTablesAndJoins (db: Database) extends Migration with LazyLogging {
  ...
  override def apply(): Unit = {
    ...
    val dbio = DBIO.seq(
      ...
      categories += Category(categoryId = Some(0), categoryName = "Food"),
      categories += Category(categoryId = Some(1), categoryName = "Car"),
      categories += Category(categoryId = Some(2), categoryName = "Sport"),
      categories += Category(categoryId = Some(3), categoryName = "Home appliance"),
      categories += Category(categoryId = Some(4), categoryName = "Medical"),
